...
Vue.component('user-contact-modal', {
  ...
  template: `
    <div ref="userContactModal" class="modal" tabindex="-1" role="dialog">
      <div class="modal-dialog modal-md" role="document">
        <div v-if="user" class="modal-content">
          <div class="modal-body">
            ...
            <span v-if="user.profile.institution"> ({{ user.profile.institution.name }})</span>
  ...
  methods: {
    ...
    sendMessage: function () {
      ...
      ajax().post('/api/contact/', {
        ...
        self.msg = '';
        self.msgSent = true;
        setTimeout(function () { self.msgSent = null; }, 4000);
      }).fail(function (err) {
        self.msgSent = false;
        setTimeout(function () { self.msgSent = null; }, 4000);
      });
  ...
  created: function () {
    this.$root.$on('show-contact-modal', this.show);
  },
  beforeDestroy: function () {
    this.$root.$off('show-contact-modal');
  }
});
...
Vue.component('user-viewer', {
  ...
  template: `
    <div v-if="users.length" class="row">
      <user-contact-modal v-if="showContactModal"></user-contact-modal>
        ...
        <table class="table table-sm">
          <tr v-for="user in filteredUsers" :key="user.id">
            <td v-if="selectedUser == user.id" @click="selectedUser = 0" class="pointer">
              <q-icon name="expand_less" class="sm mr-2"></q-icon>
              {{ user.profile.name }}
            ...
            <td v-else @click="selectedUser = user.id" class="pointer">
              <q-icon name="expand_more" class="sm mr-2"></q-icon>
              {{ user.profile.name }}
            ...
            <td v-if="showContactModal" class="sm" @click="contactUser(user)">
              <q-icon name="email" class="sm pointer text-primary"></q-icon>
            </td>
            <td class="sm">
              <a v-if="user.href" :href="user.href" target="_blank"><q-icon name="open_in_new" class="sm"></q-icon></a>
            </td>
          </tr>
        </table>
      </div>
  ...
  methods: {
    contactUser: function (user) {
      this.$root.$emit('show-contact-modal', user);
    },
