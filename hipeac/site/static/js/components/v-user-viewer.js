...
Vue.component('user-contact-modal', {
  ...
  created: function () {
    EventHub.$on('show-contact-modal', this.show);
  },
  beforeDestroy: function () {
    EventHub.$off('show-contact-modal');
  }
});
...
Vue.component('user-viewer', {
        ...
        '<table class="table table-sm">' +
          '<tr v-for="user in filteredUsers" :key="user.id">' +
            ...
            '<td v-if="showContactModal" class="sm" @click="contactUser(user)">' +
              '<icon name="email" class="sm pointer text-primary"></icon>' +
            '</td>' +
            '<td class="sm">' +
  ...
  methods: {
    contactUser: function (user) {
      EventHub.$emit('show-contact-modal', user);
    },
