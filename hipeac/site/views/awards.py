...
class TechTransferApplicationFormView(generic.FormView):
    ...
    def dispatch(self, request, *args, **kwargs):
        if request.user.profile.membership is None:
            messages.error(self.request, 'Sorry but only HiPEAC Members, Affiliate Members or '
...
class TechTransferApplicationUpdateView(TechTransferApplicationFormView, generic.edit.UpdateView):
    ...
    def get_object(self, queryset=None):
        ...
        if obj.applicant.id != self.request.user.id:
            messages.error(self.request, 'Sorry but you don\'t have the necessary permissions '
                                         'to update this Technology Transfer application.')
            raise PermissionDenied
        return obj
