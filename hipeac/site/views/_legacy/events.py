...
class EventStats(generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        def get_registrations_data(event):
            ...
            y_dict = {}
            for r in (
                Registration.objects.filter(event_id=event.id)
        ...
        current_trace = pgo.Bar(
            x=genders, y=get_gender_data(self.event), name=self.event.year, marker=dict(color=color_primary)
        )
        ...
        layout.update(
            {"margin": pgo.Margin(l=20, r=0, b=100, t=20, pad=5), "barmode": "group", "xaxis": dict(tickangle=-90)}
        )
