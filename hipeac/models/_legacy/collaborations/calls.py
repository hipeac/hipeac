...
class Call(models.Model):
    ...
    manager = models.ForeignKey(
        get_user_model(), null=True, related_name="managed_collaboration_calls", on_delete=models.SET_NULL
    )
    ...
    def is_editable_by_user(self, user):
        ...
        if self.status == self.STATUS_REVIEWING:
            return user.is_staff or user.id == self.manager.id or self.reviewers.filter(id=user.id).exists()
        else:
            return user.is_staff or user.id == self.manager.id
...
class Application(models.Model):
    ...
    host_institution_string = models.CharField(
        "Host institution (alt)",
        max_length=250,
        null=True,
        blank=True,
    ...
    statement = models.TextField(
        "About the collaboration", help_text="Has there been research collaboration between the institutions before?"
    )
    project_file = models.FileField(upload_to=get_asset_path, null=True, blank=True, help_text="Maximum 1 page!")
    ...
    created_by = models.ForeignKey(
        get_user_model(), null=True, related_name="collaborations", on_delete=models.SET_NULL
    )
    ...
    def is_editable_by_user(self, user):
        ...
        return user.is_staff or user.id == self.created_by.id or user.id == advisor_id or user.id == host_id
    ...
    def is_viewable_by_user(self, user):
        ...
        return (
            user.is_staff
            or user.id == self.created_by.id
            or user.id == advisor_id
            or user.id == host_id
            or user.id == self.call.manager.id
            or self.call.reviewers.filter(id=user.id).exists()
        )
...
class Review(models.Model):
    ...
    reviewer = models.ForeignKey(
        get_user_model(), null=True, related_name="collaboration_reviews", on_delete=models.SET_NULL
    )
