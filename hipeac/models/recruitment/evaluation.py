...
class JobEvaluation(EditorMixin, models.Model):
    ...
    selected_user = models.ForeignKey('auth.User', null=True, blank=True, on_delete=models.SET_NULL,
                                      related_name='selected_jobs', help_text='Internal use only.')
...
def job_evaluation_post_save(sender, instance, created, *args, **kwargs):
    if created:
        send_internal_email(
            to=f'HiPEAC Recruitment <recruitment@hipeac.net>',
